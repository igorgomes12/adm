...
export const ModalBudgetDelete = () => {
  const { isOpen, id, onClose } = useBudgetStore();
  ...
  const { mutate } = useMutation({
    ...
    onSuccess: () => {
      toast.success("Orcamento excluído com sucesso!", {
        theme: "dark",
        icon: <FaRocket />,
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-50 backdrop-blur-sm">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md mx-4 sm:w-2/3 lg:w-1/3">
        <TitleMessageDelete
          title="Tem certeza que deseja excluir o Orcamento?"
          message={`Esta ação não pode ser desfeita. O Orcamento com ID ${id} será permanentemente removido.`}
        />
